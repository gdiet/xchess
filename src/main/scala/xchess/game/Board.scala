...
case class Board(size: Square, map: Map[Square, PieceOnMap]):
  ...
  def executeMove(from: Square, to: Square, freezeUntil: GameTime): Option[(board: Board, movedTo: Square)] =
    ...
    else map.get(from).flatMap { case (piece, _) =>
      tryMove(piece, from, to).flatMap(target =>
        map.get(target) match
          case Some((targetPiece, _)) if targetPiece.isWhite == piece.isWhite =>
            None // piece of the same color at target, can not move there
          case _ => // includes the capturing case
            if piece.isPawn && target.row == Row(0) || target.row == size.row then // promote pawn to queen
              Some(copy(map = map - from + (target -> (piece.promote, freezeUntil))) -> target)
            else
  ...
  private def tryMove(piece: Piece, from: Square, to: Square): Option[Square] = {
    ...
    def pawnMoveOneLogic: Option[Square] =
      ...
      else if cols == 0 && map.contains(to) then None // must not capture a piece on the same col
      else if Math.abs(cols) == 1 && !map.contains(to) then None // must capture a piece if moving diagonally
      else Some(to) // yay
    ...
    def firstOccupiedSquare: Option[Square] =
      ...
      def startingAt(square: Square): Option[Square] =
        ...
        else if map.contains(square) then Some(square) // found a piece
    ...
    piece.value.toUpper match
      ...
      case 'P' => // Pawn not moved logic
        ...
        else if map.contains(to) then None // can not capture forward
