...
def executeAllScheduledMoves(game: Game, gameTime: GameTime): Game =
  executeNextScheduledMove(game, gameTime) match
...
def executeNextScheduledMove(game: Game, gameTime: GameTime): Option[Game] =
  import game.*
  @annotation.tailrec
  def recurse(moves: Iterator[(Square, Square)]): Option[Game] = {
    moves.nextOption match
      ...
      case Some(from, to) => board.map.get(from) match
        ...
        case Some(_, frozenUntil) if frozenUntil > gameTime => recurse(moves) // piece is frozen, skip this move
        case Some((piece, _)) => Some(handleMoveCommand(game, piece, from, to, gameTime))
  }
  recurse(game.plannedMoves.iterator)
